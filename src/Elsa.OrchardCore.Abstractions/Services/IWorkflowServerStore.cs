...
namespace Elsa.OrchardCore.Services
{
    public interface IWorkflowServerStore
    {
        ...
        Task<WorkflowServer?> GetByIdAsync(string id, in CancellationToken cancellationToken = default);
        Task DeleteAsync(WorkflowServer workflowServer, CancellationToken cancellationToken);
    }
}